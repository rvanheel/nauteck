...
public sealed class DealerQueryHandler(IDbConnection dbConnection) : IRequestHandler<DealerQuery, IEnumerable<data.Entities.Dealer.Dealer>>
{
    public Task<IEnumerable<data.Entities.Dealer.Dealer>> Handle(DealerQuery request, CancellationToken cancellationToken)
    {
        return dbConnection.QueryAsync<data.Entities.Dealer.Dealer>($"SELECT * FROM {DbConstants.Tables.Dealer}");
    }
}